...
| DS3 | ArtBench-10 (жанровые подмножества) | 2 × **500** изображений *(reduced)*     | CC-BY          | Phase 4 FID reference    | [ ] скачать по жанру |
| DS4 | Block analysis prompts           | 200 пар (object, color)                     | —              | Phase 0                  | [ ] создать |
| DS5 | DreamBooth canonical subjects    | **8** предметов: dog (~30 img) + 7 для DS8  | Apache 2.0     | Phase 1b; **DS8 content** | [ ] скачать |
| DS6 | Limitations prompts              | 5 + 5 = 10 промптов                         | CC-BY (COCO)   | Phase 4b (L01, L03)      | [ ] создать |
| DS7 | Center-cropped styles            | 8 изображений                               | public domain  | Phase 4b (L02)           | [ ] сгенерировать |
| DS8 | B-LoRA paired eval (§5.1)        | **8** объектов × **8** стилей → **50** пар *(reduced)* | DS1 + DS5 | **plan.md** Phase **5.2**; опц. Phase 6 M02 | [ ] собрать |
...
**Назначение**:
- Phase 1b: стабильный **single-subject** контент для поиска Θ_content (**dog** как канонический набор ~30 изображений).
...
- **Готово**: DS1, DS2 (2 / 8).
- **Создать**: DS4 (генерируется автоматически `block_analysis.py`, зафиксировать JSON-артефакт); DS6 (10 промптов прямо из этого файла, ~5 мин); DS7 (скрипт кропа DS1); **DS8** (`build_eval_pairs.py` + JSON на 50 пар из DS1 × DS5).
- **Скачать**: DS5 (**8 предметов**: `dog` + `cat`, `backpack`, `bowl`, `can`, `clock`, `vase`, `bear` из `google/dreambooth`); DS3 (по **500 img** жанра `impressionism` и `post_impressionism` с HF `Doub7e/ArtBench-10`, организовать в подпапки).
- **Решения (зафиксированы 2026-05-06)**:
  - DS8 → *reduced protocol* (Nc=8, Ns=8, 50 пар) — **не менять без пересмотра**
  - DS3 → 500 img/жанр (approximate FID) — упомянуть в тексте ВКР
  - DS5 → 8 предметов вместо 23
